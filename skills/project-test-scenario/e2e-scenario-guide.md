...
> Used by the `project-test-scenario` skill during scenario generation.
...
## 5. Integration with project-test-run Skill

### 5.1 Scenario-to-Test Mapping

The E2E scenarios generated by `project-test-scenario` are designed to be directly executable by the `/project-test-run` skill:

| Scenario Element | project-test-run Execution |
|-----------------|----------------------|
...
```
1. /project-test-scenario auth     <- Generate scenarios
2. /project-test-run auth               <- Execute scenarios in browser
